            ...
            <div className='flex flex-col justify-center h-[400px] relative top-1/2 transform -translate-y-1/2'>
                ...
                <div className='flex justify-center mb-0'>
                    <Image
                        ...
                        alt={project.title}
                    />
                    {heading(project.inProgress)}
                </div>

                {/* Description container */}
                <div className='h-[150px] my-2'>
                    <h3 className='text-lg font-medium pt-1 pb-1'>{project.descTitle}</h3>
                    <p className='text-xs custom-scrollbar'>{truncateText(project.desc, maxDescLength)}</p>
                </div>
            </div>

            {/* Buttons container */}
            <div className='flex justify-evenly absolute bottom-0 left-0 w-full p-2 z-20'>
                <a href={project.rep} className='hover:bg-white p-2 text-sky-400 border-2 border-sky-400 rounded-lg' target='_blank'>
                    github
                </a>
                {button(project.isPublished, project.live)}
            </div>
        </div>
    );
}
